...
```c++
MT::ThreadPool::ThreadPool(size_t NUM_THREADS) {
    paused.store(true);
    stopped.store(false);
    last_task_id = 0;
    ...
    for (size_t i : std::ranges::iota_view(static_cast<size_t>(0), NUM_THREADS)) {
		...
		MT::Thread& th = threads.back();
		th._thread = std::move(std::thread(&ThreadPool::run, this, std::ref(th)));
		th.is_working.store(false);
    }
}
```
...
Позднее мной был реализован метод, который должен был считать количество загруженных потоков, однако работает он некорректно, причины этого мне выяснить не удалось...
...
И файлик с задачами, не буду вдаваться в подробности реализации, кратко опишу их назначение:
